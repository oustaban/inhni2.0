...
var MM = {
    ...
    loadLayout: function() {
        ...
        var tpl = MM.tpl.render($('#add-site_template').html(), {sites: sites});
        $('#add-site').html(tpl);
        ...
        var tpl = MM.tpl.render($('#forgot-password_template').html());
        $('#forgot-password').html(tpl);
        ...
        //$('#forgot-password form').on('submit', this.forgotPassword);
        ...
        $('#add-site, #manage-accounts, #forgot-password').css('height', $(document).innerHeight());
    ...
    _displayAddSite: function() {
        ...
        if (sites.length > 0) {
            $('#manage-accounts-link').show();
        } else {
            $('#manage-accounts-link').hide();
        }
    ...
    _displayForgotPassword: function() {
        $('#manage-accounts').css('display', 'none');
        $('#forgot-password').css('display', 'block');
        $('#add-site').css('display', 'none');
    },

    _displayMenuCourse: function(courseId) {
        
        var coursesloaded = MM.db.where("courses",{courseid : parseInt(courseId), siteid: MM.config.current_site.id});
        var course = coursesloaded[0].toJSON();
        MM._showMainAppPanels();
        $('body').removeClass();
        MM.log('_displayMenuCourse:'+course.id+'/'+course.managertype);
        values = {
            user: {
        ...
        var output = MM.tpl.render($('#menu-course_template').html(), values);
    ...
    checkSite: function(e, protocol) {
        ...
        if (siteurl.indexOf('http://localhost') == -1 && !MM.validateURL(siteurl)) {
            msg = MM.lang.s('siteurlrequired');
            $('.invalid-text').html(msg);
            $('.invalid-text').show();
    ...
    _saveTokenSuccess: function(site, token) {
        ...
        if (!validMoodleVersion) {
            $('.invalid-text').html(MM.lang.s('invalidmoodleversion') + "2.4");
            $('.invalid-text').show();
        ...
        if (typeof site.downloadfiles != "undefined" && site.downloadfiles === 0) {
            ...
            $('.invalid-text').html(MM.lang.s('cannotdownloadfiles'));
            $('.invalid-text').show();
            return;
        }
    ...
    loadRoutes: function() {
        var routes = [
            ...
            ['forgot-password', 'forgot_password', MM._displayForgotPassword],
            ['menu-course/:courseid', 'menu_course', MM._displayMenuCourse],
        ];
    ...
    popConfirm: function(text, callBack, cancelCallBack) {
        ...
        options.buttons[MM.lang.s('no')]['style'] = "btn--secondary-alert";
        
        options.buttons[MM.lang.s('yes')] = function() {
            MM.widgets.dialogClose();
            callBack();
        };
        options.buttons[MM.lang.s('yes')]['style'] = "btn--alert";
