...
MM.widgets = {
    ...
    dialog: function(text, options) {
        if (!options) {
            ...
            options.buttons[MM.lang.s('close')]['style'] = "btn btn--alert";
        }
        if (!options.buttons) {
            options.buttons = {};
            options.buttons[MM.lang.s('close')] = MM.widgets.dialogClose;
        ...
        if (options.title)
            $("#app-dialog .modalHeader").html(options.title + '<br/>');
        
        $("#app-dialog .modalHeader").append(text);
        ...
        if (options.buttons) {
            ...
            $.each(options.buttons, function (key, value) {
                if (options.buttons[key]['style'])
                    buttons += "<button class=\"" + options.buttons[key]['style'] + "\"> " + key + " </button>";
                else
                    buttons += "<button class=\"modal-button-" + els + "\"> " + key + " </button>";
                els++;
            });
